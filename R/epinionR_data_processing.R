...
epinion_apply_dictionary = function(x, dict){
  ...
  for(i in seq_len(nrow(references))){
    labelled::val_label(x[[references$variable[i]]], references$value[i]) = references$label[i]
  }
  ...
  for(i in seq_len(nrow(categorical_vars))){
    if (class(x[[categorical_vars$variable[i]]])[1] != "factor") {
      labelled::val_label(x[[categorical_vars$variable[i]]], categorical_vars$value[i]) = categorical_vars$label[i]
    }
  }
